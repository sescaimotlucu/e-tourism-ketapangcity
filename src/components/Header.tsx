              ...
              {navItems.map((item, index) => (
                item.isLink ? (
                  ...
                  <button
                    key={item.label}
                    onClick={() => handleNavClick(item)}
                    className="block w-full text-left px-6 py-3 text-white hover:text-golden-beige hover:bg-red-soft/20 transition-all duration-300 font-poppins font-medium rounded-xl mx-2 animate-fade-in-left"
                    style={{ animationDelay: `${index * 0.1}s` }}
                  >
                    {item.label}
                  </button>
